...
<div class="absolute inset-0 bg-black/80" style="z-index:-1" />
...
- **Шаг 0: UI, токены и стоимость** - куда уходят запросы и как отслеживать расход
- Кейс 1: планирование поездок - от визовых документов до маршрутов
- Кейс 2: работа с презентациями через код
- Кейс 3: email-агент - подключение почты, ответы и ведение переписки
- Кейс 4: генерация изображений по материалам из слайдов
- Кейс 5: рабочая задача по управлению планами (если успеем)
...
- "Нужно спланировать поездку в город N"
- "Подготовь данные для визы"
...
3. сам ответ
4. при любых правках - commit + push после подтверждения что все ок
...
- Например: *«Я хочу пообщаться с человеком X - я буду на его докладе?»*
- Агент откроет сохранённый план — без повторного сбора информации
...
<div class="flex justify-center">
  <img
    ...
    class="mt-1 mx-auto rounded-lg border border-white/10 max-h-[48vh] w-auto"
  />
</div>
...
<div class="flex justify-center">
  <img
    ...
    class="mx-auto rounded-lg border border-white/10 max-h-[48vh] w-auto"
  />
</div>
...
- создание структуры новой презентации по задаче (заготовка)
- массовые правки (переименовать, поменять порядок, поменять дизайн)
- правка структуры и содержания слайдов
- расстановка ссылок, генерация QR кодов и тд
- перенос частей из старых презентаций, например титула, прощания и т.п.
...
<img
  src="slides_case_1.png"
  alt="Первый этап диалога про добавление анонса вебинара в презентацию"
  class="w-full max-h-[48vh] rounded-lg border border-white/10 shadow-lg object-contain"
/>

<img
  src="slides_case_2.png"
  alt="Второй этап диалога про правку слайда с анонсом вебинара"
  class="w-full max-h-[48vh] rounded-lg border border-white/10 shadow-lg object-contain"
/>

<img
  src="slides_case_3.png"
  alt="Финальный этап диалога про анонс вебинара и публикацию правки"
  class="w-full max-h-[48vh] rounded-lg border border-white/10 shadow-lg object-contain"
/>
...
<div class="h-full flex items-center justify-center">
  <img
    src="slides_case_result.png"
    alt="Результат правки слайда с анонсом следующего вебинара"
    class="max-w-full max-h-[78vh] rounded-lg border border-white/10 shadow-lg object-contain"
  />
</div>
...
<div class="mt-3 rounded-xl border border-cyan-500/30 bg-cyan-900/20 p-4">
  <div class="text-xs uppercase tracking-wide text-cyan-300">Следующая неделя</div>
  <div class="mt-1 text-2xl font-semibold text-white">GPT2GIGA: Интеграция AI-агентов с GigaChat</div>
  <div class="mt-2 text-xs leading-snug text-gray-300">
    GPT2GIGA - open source прокси от команды GigaChain, который перенаправляет запросы OpenAI API и Anthropic API в GigaChat. Приложения продолжают работать без изменений в коде - достаточно сменить базовый URL.
  </div>
</div>

<div class="mt-4 grid grid-cols-[1.15fr_0.85fr] gap-4 text-xs">

<div class="rounded-xl border border-white/10 bg-white/5 p-4">

### На вебинаре разберем

<div class="grid grid-cols-2 gap-x-4 gap-y-1 leading-snug">

<div>- что такое GPT2GIGA и какие задачи он решает</div>
<div>- установка и запуск: pip, uv install, Docker Compose</div>
<div>- переменные окружения и аргументы запуска прокси</div>
<div>- эндпоинты: `chat/completions`, `responses`, `messages`, `embeddings`</div>
<div>- примеры с OpenAI Agents SDK</div>
<div>- наблюдаемость через mitmproxy</div>
<div class="col-span-2">- AI-инструменты для разработки: Claude Code, Codex, KiloCode, Cursor</div>
...
<div class="rounded-xl border border-emerald-500/30 bg-emerald-900/20 p-4">
  <div class="text-[11px] uppercase tracking-wide text-emerald-300">Когда</div>
  <div class="mt-1 text-2xl font-semibold text-white">18 марта, 10:00</div>
  <div class="mt-1 text-xs text-gray-300">Среда</div>
</div>

<div class="rounded-xl border border-amber-500/30 bg-amber-900/20 p-4 text-xs leading-snug text-amber-100">
  После вебинара вы будете знать, как развернуть GPT2GIGA, какие инструменты и фреймворки с ним работают, и сможете сразу применить это в своих проектах на реальных примерах.
</div>
...
Важно: боту нужен отдельный почтовый ящик - не ваш личный. Я использую выделенный Gmail-аккаунт, на который настроены пересылки и доступы. Для тех, кто пользуется Telegram, почта тоже остается важным элементом: Telegram-боты не могут отправлять письма моментально и иногда путают людей. Бот обычно примерно понимает, кто хозяин, а кто нет, но нередко начинает делать задачу от одного человека, а результат присылает другому.
</div>
...
# Кейс 3: почта + проверка презентации

<img src="mail_case3.png" class="mt-4 rounded-lg border border-white/10 max-h-[460px] mx-auto" />

<div class="mt-3 text-sm text-gray-400">
Здесь вход тоже пришел через почту: я переслал агенту анонс вебинара и спросил, что именно там уже обещано рассказать. После этого агент сверил анонс с актуальной `Slidev`-презентацией и проверил, что ключевые разделы действительно покрыты.
</div>

<div class="mt-3 rounded-lg border border-cyan-500/30 bg-cyan-900/15 p-3 text-sm text-cyan-100">
Получается удобная связка: почта работает как канал постановки задачи, а дальше агент умеет открыть репозиторий со слайдами, посмотреть текущую версию презентации и подсветить, что уже есть, а что еще стоит добавить, чтобы не забыть обещанное в анонсе.
</div>

---

# Кейс 3: отправка артефактов по почте

<div class="mt-4 grid grid-cols-[0.9fr_1.1fr] gap-6 items-start">

<div>

<div class="p-4 rounded-lg bg-gray-800/60 text-sm">

- иногда файлы удобнее отправить на почту, а не через Telegram
- например, когда их много, нужен особый формат или размер больше 5 МБ

</div>

<div class="mt-3 rounded-lg border border-emerald-500/30 bg-emerald-900/15 p-3 text-sm text-emerald-100">
Простой сценарий: агент собирает артефакты и отправляет их письмом как вложения.
</div>
...
<img src="email_assets.png" class="rounded-lg border border-white/10 max-h-[420px] mx-auto" />
...
# Кейс 4: постер по данным из слайдов
...
# Кейс 5: конвейер апробации GigaChat

<div class="mt-4 grid grid-cols-[1.1fr_0.9fr] gap-5 text-sm">

<div class="flex flex-col gap-3">

<div class="rounded-xl border border-cyan-500/30 bg-cyan-900/15 p-4">

### Как это работает

<v-clicks>

- В чате появляется новый интересный open source проект
- Я пересылаю находку Риззи и прошу быстро объяснить, что это и зачем
- Она раскладывает по полочкам: use case, зрелость проекта, как он обычно встраивается в стек
- Если вижу потенциал - прошу добавить проект в конвейер апробации

</v-clicks>

</div>

<div class="rounded-xl border border-white/10 bg-white/5 p-4 text-xs leading-relaxed text-gray-300">
Это удобный фильтр перед ручной проверкой: сначала AI помогает понять, стоит ли вообще тратить время команды, а уже потом мы запускаем совместимость с GigaChat в рабочем процессе.
</div>
...
### Что попадает в конвейер

<div class="space-y-2 text-xs leading-snug text-emerald-50">
  <div>- проверка базового сценария интеграции с GigaChat</div>
  <div>- разбор, где нужен OpenAI-compatible слой, а где все работает нативно</div>
  <div>- фиксация ограничений, обходных путей и полезных настроек</div>
</div>

</div>

<div class="rounded-xl border border-amber-500/30 bg-amber-900/20 p-4 text-xs leading-relaxed text-amber-100">
По сути, Риззи помогает превратить случайную идею из чата в системный backlog на апробацию: меньше хаоса, быстрее приоритизация, больше реальных экспериментов.
</div>
...
<div class="mb-3 text-center text-sm text-cyan-200">
Первичный разбор проекта
</div>

<img src="pipeline1.png" class="w-full rounded-lg border border-white/10 shadow-lg max-h-[400px] object-contain bg-black/20" />

</div>

<div>

<div class="mb-3 text-center text-sm text-emerald-200">
Фиксация в конвейере апробации
</div>

<img src="pipeline2.png" class="w-full rounded-lg border border-white/10 shadow-lg max-h-[400px] object-contain bg-black/20" />
...
<img src="pipeline_result.png" class="w-full rounded-lg border border-white/10 shadow-lg max-h-[400px] object-contain mx-auto bg-white" />

</div>

<div class="mt-3 text-sm text-gray-400">
После обсуждения в чате задача не теряется: агент фиксирует ее в репозитории как конкретный элемент конвейера апробации.
</div>

---

# Мини-пример: браузер + документы

<div class="mt-4 grid grid-cols-[1.15fr_0.85fr] gap-6 items-start">

<div>

<video src="browser_control.mov" controls muted playsinline class="w-full rounded-lg border border-white/10 shadow-lg max-h-[58vh] bg-black"></video>

<div class="mt-2 text-xs text-gray-500">
Короткий фрагмент: агент сам кликает по интерфейсу в браузере, но решение принимает с учетом локального контекста.
</div>
...
1. агент получает короткую задачу
2. открывает нужную страницу и проходит шаги в браузере
3. параллельно сверяется с документами в репозитории
4. использует из них нужные данные для следующего действия

</div>

<div class="p-4 rounded-lg border border-cyan-500/30 bg-cyan-900/15 text-cyan-100">

Важно, что браузер тут - не отдельный фокус, а просто еще один инструмент. Полезность появляется, когда агент совмещает действия в UI с контекстом из файлов и документов.

</div>

<div class="p-4 rounded-lg border border-emerald-500/30 bg-emerald-900/15 text-emerald-100">

Это и есть формат мини-примеров: короткая демонстрация показывает не только "умеет нажимать кнопки", а "умеет дойти до результата, опираясь на сохраненные данные".
...
<div class="text-gray-400 mb-4">Все материалы - в Telegram-канале</div>

<div class="mt-8 rounded-xl border border-sky-500/30 bg-sky-900/20 px-6 py-6">
  <div class="flex items-center justify-center gap-8">
    <img
      src="https://api.qrserver.com/v1/create-qr-code/?size=220x220&data=https%3A%2F%2Ft.me%2Frobofuture"
      alt="QR-код на Telegram-канал RoboFuture"
      class="w-[180px] h-[180px] rounded-lg bg-white p-2"
    />
    <div class="text-center">
      <div class="text-lg text-white font-semibold mb-3">RoboFuture</div>
      <a href="https://t.me/robofuture" target="_blank" class="text-2xl text-sky-300 font-bold">
        https://t.me/robofuture
      </a>
      <div class="mt-4 text-sm text-gray-200 max-w-[520px]">
        Там собраны все ссылки на первую часть, и там же выложим запись этого вебинара.
      </div>
    </div>
  </div>
</div>