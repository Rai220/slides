...
theme: penguin
colorSchema: dark
title: "OpenClaw: универсальный ИИ-помощник"
fonts:
  sans: 'Open Sans'
  provider: google
class: text-left
...
Универсальный ИИ-помощник с открытым исходным кодом
</div>
...
220K+ GitHub Stars • MIT License • TypeScript
</div>
...
<div class="grid grid-cols-2 gap-x-8 gap-y-2 mt-4">
  ...
  <div class="flex items-center gap-2"><span class="text-blue-400">5.</span> Запуск на GigaChat</div>
  <div class="flex items-center gap-2"><span class="text-blue-400">2.</span> Архитектура: простой loop</div>
  <div class="flex items-center gap-2"><span class="text-blue-400">6.</span> Моё видение</div>
  <div class="flex items-center gap-2"><span class="text-blue-400">3.</span> Системный промпт и память</div>
  <div class="flex items-center gap-2"><span class="text-blue-400">7.</span> Личный опыт</div>
  <div class="flex items-center gap-2"><span class="text-blue-400">4.</span> Skills, MCP и Heartbeat</div>
</div>
...
- Запускается **на вашем устройстве** - macOS, Linux, Windows, VM
...
- **MIT лицензия**, 220K+ звёзд на GitHub
- **Автор**: Peter Steinberger - 13 лет строил PSPDFKit (1B+ устройств), продал, вернулся к коду
- **История имён**: WhatsApp Relay → Clawd → Moltbot → **OpenClaw** (26.02.2026) → ???
...
На каждое простое «Привет» создается запрос на ~1500 строк JSON. Основной объём: системный промпт + описания тулов.
...
# Как работает подключение к GigaChat

<img src="gpt2giga-schema.png" class="rounded-lg bg-white p-3 mb-4 h-44 object-contain" />

<v-clicks>

- OpenClaw использует **OpenAI-совместимый** формат (`/chat/completions`)
- GigaChat API имеет **собственный формат** запросов
- **gpt2giga** - прокси-транслятор, работает на той же виртуалке
- OpenClaw → `OpenAI /chat/completions` → **gpt2giga** → `GigaChat /chat/completions` → **GigaChat API**
- Для OpenClaw это прозрачно - он «думает», что общается с OpenAI

</v-clicks>

<div v-click class="mt-3 p-3 bg-gray-800/50 rounded-lg text-xs">
<b>gpt2giga</b> транслирует формат запросов и ответов на лету: маппинг ролей, tool calls, streaming. OpenClaw не знает, что за прокси стоит GigaChat - подключение выглядит как обычный OpenAI-провайдер.
</div>

---

# Настройка OpenClaw + GigaChat

**1. Редактируем конфиг провайдеров:**

```bash
nano /root/.openclaw/openclaw.json
```

**2. Добавляем GigaChat через gpt2giga:**
...
`baseUrl` указывает на локальный **gpt2giga**, который проксирует запросы в GigaChat API. Авторизация настраивается в gpt2giga.

</div>

<img src="gigachat-crab.png" class="absolute right-2 top-24 h-32 rounded-lg shadow-lg border border-white/10" />
...
<div class="flex justify-center items-center h-[60vh]">
  <div class="text-center">
    <h1 class="text-3xl font-bold text-gray-300">Живое демо</h1>
    <div class="mt-4 text-xl text-gray-500">GigaChat</div>
  </div>
</div>
...
<div class="flex-shrink-0">
  <img src="kinonavigator-telegram-chat.png" class="w-[250px] h-auto rounded-lg shadow-lg border border-white/10" />
</div>
...
<div class="text-gray-400 mb-6">Готов ответить на вопросы</div>
...
<div class="mt-6 flex items-center justify-center gap-4">
  <img src="/krestnikov_big.png" class="w-14 h-14 rounded-full" />
  <div class="text-left">
